                        ...
                        <div class="product-payment-inner-st">
                            ...
                            if(mysqli_num_rows($result)==0) 
                            {
                                ...
                                <?php 
                                    ...
                                    $requet="SELECT * FROM exercice WHERE type='Eval' and source='$id_eval' order by id_exo";
                             ...
                             {
                                $evalua_fait=mysqli_fetch_assoc($result);

                                 $valeur=(($evalua_fait["note_el"]*20)/$evalua_fait["sur"]);
                                    $valeur=number_format($valeur,2);

                                    if ($valeur<11) 
                                    {
                                        $coul_e="red";
                                        $observation=" Compétences non acquise <br> Tu dois Faire la prochaine évaluation ";
                                    }
                                    elseif ($valeur<15) 
                                    {
                                        $coul_e="blue";
                                        $observation=" Compétences en cours d'acquisition <br> Repasse la prochaine évaluation";
                                    }
                                    else
                                    {   //on sélectionne la certification
                                        $requet="SELECT id_cer FROM certification WHERE eval='$id_eval' and eleve='$ide'";
                                        $resulta=mysqli_query($con,$requet) or die("impossible d'exécuter la requet");
                                        $certif=mysqli_fetch_assoc($resulta);
                                        $cer=$certif["id_cer"];

                                        $coul_e="green";
                                        $observation=" FELICITATION !!! Compétences acquise ! <br> Tu as obtenu une Certification";


                                    }

                                     echo '<div class="text-center m-b-md custom-login"><h2>Note <b><span style="color:'.$coul_e.'">'.$valeur.'</b></span> / 20</h2></div>
                                            <div class="text-center m-b-md custom-login"><h4><b><span style="color:'.$coul_e.'">'.$observation.'</b></span></h4></div>';  
                                            if (isset($cer)) 
                                            { ?>
                                                <a href="certification.php?cer=<?=$cer ?>" target="_blank"><h4 style="text-align: center;color: ">clique ici pour voir ta certification</h4></a><br>
                                             <?php }

                                 //on sélectionne l'exercice fait
                                    $requet="SELECT re.exo, re.note, re.sur, re.date, ex.intitule, ex.enoncer FROM rep_exo re, exercice ex WHERE ex.id_exo=re.exo and re.eval='$id_eval' and re.eleve='$ide' order by re.exo";
                                    $resultat=mysqli_query($con,$requet) or die("impossible d'exécuter la requet");
                                     //$exercice=mysqli_fetch_assoc($result);
                          
                                    $i=0;
                                    $e=0;

                                while ($exo_fait=mysqli_fetch_assoc($resultat)) 
                                {
                                    $exo=$exo_fait["exo"];
                                        $e++;
                                        ?>
                                    ...
                                    <table>
                                        <tr>
                                            <td style="text-align:center"><div class=" m-b-md custom-login"><h3 style="color:"><b><u>Exercice <?php echo $e.'</u> </b>: '.$exo_fait["intitule"]; ?></h3></div></td>
                                            <td style="text-align:center"><h4>  </h4></td>
                                            <td style="text-align:center"><h4><span style="color:<?php echo $couleur; ?>"><?php echo $exo_fait["note"].'</span> / '.$exo_fait["sur"]?></h4>
                                                                        <h4><span style="color:<?php echo $couleur; ?>"><?php echo $appreciat; ?></span></h4>
                                                                       </td>
                                        </tr>
                                    </table>

                                    <?php
                                    echo $exo_fait['enoncer'];
                                    echo '<br>';
                             ...
                             <?php
                                if (isset($_POST['submit']) and $soumit==1) 
                                {
                                    ...
                                    $sur=0;
                                    $noteEval=0;                        // note de l'évaluation
                                    $sur_eval=0;
                                    ...
                                    { 
                                        ...
                                        // si l'exercice de la question est != de celui de la question suivante on insère l'exo précédent
                                        if (isset($fin_exo) and $fin_exo!=$ex) 
                                        {
                                            $requet="INSERT INTO rep_exo (eleve, exo, note, sur, eval) values ('$ide', '$fin_exo', '$total', '$sur', '$id_eval')";
                                            $result=mysqli_query($con,$requet) or die("impossible d'exécuter la requet exo"); 
                                            $noteEval=$noteEval+ $total;
                                            $sur_eval=$sur_eval+$sur;
                                            $total=0;                          
                                            $sur=0;
                                        }
                                        ...
                                        $fin_exo=$ex;       // va servir à vérifier si on est sur le même exo

                                        if ($i==$n) 
                                        {
                                            $requet="INSERT INTO rep_exo (eleve, exo, note, sur, eval) values ('$ide', '$ex', '$total', '$sur', '$id_eval')";
                                            $result=mysqli_query($con,$requet) or die("impossible d'exécuter la requet exo"); 
                                            $noteEval=$noteEval+ $total;
                                            $sur_eval=$sur_eval+$sur;
                                            $total=0;                          
                                    ...
                                    $temps=$evaluat["duree_eval"];
                                    $requet="INSERT INTO eval_fait (eleve, eval, note_el, sur, temps) values ('$ide', '$id_eval', '$noteEval', '$sur_eval', '$temps')";
                                    $result=mysqli_query($con,$requet) or die("impossible d'exécuter la requet exo");

                                     $note_f=(($noteEval*20)/$sur_eval);
                                    $note_f=number_format($note_f,2);
                                    if ($note_f>=15) 
                                    {
                                        $nom_evalua=$disp["discipline"];
                                        $requet="INSERT INTO certification (eleve, eval, note, ue, nom_ue, classe) values ('$ide', '$id_eval', '$note_f', '$ue', '$nom_evalua', '$classe')";
                                        $result=mysqli_query($con,$requet) or die("impossible d'exécuter la requet certif");
                                    }
                                    $soumit=0;
